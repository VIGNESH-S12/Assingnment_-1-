...
# Insert the new data
if result[0] == 0:
    df = pd.read_csv(r"C:\Users\Shruthilaya\GUVI\data\traffic_stops - traffic_stops_with_vehicle_number.csv")
    ...
    query = "INSERT INTO Police VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
st.subheader("A Python-SQL Digital Ledger for Police Post Logs")
...
#  Analytics
st.header("Insights")
st.subheader("Violation Types Bar Graph")
fig3, ax3 = plt.subplots(figsize=(10, 4))
violation_counts.plot(kind='bar', x='Violation Type', y='Count', ax=ax3, color='skyblue')
ax3.set_ylabel("Count")
ax3.set_title("Frequency of Each Violation Type")
...
query_categories = {
    ...
    "Demographic-Based": {
        "Which driver age group had the highest arrest rate?":
            "SELECT driver_age, COUNT(*) AS total_stops, SUM(is_arrested) AS total_arrests, ROUND(SUM(is_arrested)/COUNT(*) * 100, 2) AS arrest_rate FROM Police GROUP BY driver_age ORDER BY arrest_rate DESC LIMIT 10",
        ...
        "Which race and gender combination has the highest search rate?":
            "SELECT driver_race, driver_gender, COUNT(*) AS total, SUM(search_conducted) AS searches, ROUND(SUM(search_conducted)/COUNT(*) * 100, 2) AS search_rate FROM Police GROUP BY driver_race, driver_gender ORDER BY search_rate DESC LIMIT 10"
    },
    "Time & Duration Based": {
        "What time of day sees the most traffic stops?":
            "SELECT HOUR(stop_time) AS hour_of_day, COUNT(*) AS stop_count FROM Police GROUP BY hour_of_day ORDER BY stop_count DESC",
        "What is the average stop duration for different violations?":
            "SELECT violation, ROUND(AVG(CAST(SUBSTRING_INDEX(stop_duration, ' ', 1) AS UNSIGNED)), 2) AS avg_duration FROM Police GROUP BY violation ORDER BY avg_duration DESC",
        "Are stops during the night more likely to lead to arrests?":
            "SELECT CASE WHEN HOUR(stop_time) BETWEEN 20 AND 23 OR HOUR(stop_time) BETWEEN 0 AND 5 THEN 'Night' ELSE 'Day' END AS time_period, COUNT(*) AS total_stops, SUM(is_arrested) AS arrests, ROUND(SUM(is_arrested)/COUNT(*) * 100, 2) AS arrest_rate FROM Police GROUP BY time_period"
    },
    "Violation-Based": {
        "Which violations are most associated with searches or arrests?":
            "SELECT violation, SUM(search_conducted) AS total_searches, SUM(is_arrested) AS total_arrests FROM Police GROUP BY violation ORDER BY total_searches DESC, total_arrests DESC LIMIT 10",
    ...
    "Location-Based": {
        "Which countries report the highest rate of drug-related stops?":
            "SELECT country_name, COUNT(*) AS total_stops, SUM(drugs_related_stop) AS drug_related, ROUND(SUM(drugs_related_stop)/COUNT(*) * 100, 2) AS drug_rate FROM Police GROUP BY country_name ORDER BY drug_rate DESC LIMIT 10",
        "What is the arrest rate by country and violation?":
            "SELECT country_name, violation, COUNT(*) AS total_stops, SUM(is_arrested) AS arrests, ROUND(SUM(is_arrested)/COUNT(*) * 100, 2) AS arrest_rate FROM Police GROUP BY country_name, violation ORDER BY arrest_rate DESC LIMIT 10",
